...
public class   ImageServlet extends HttpServlet {
    //重写方法 Alt + Insert --> Overside...

    /**
     * 查看图片属性：即能查看所有，又能查看指定
     * @param req
     * @param resp
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       /* //HttpServletRequest req     请求 包含方法，url,各种 header,body
        // HttpServletResponse resp  响应 包含状态码，各种 header,body

        //req 对象中包含了请求中的所有信息
        //resp 对象要生成的结果就放到里面去
        //当前这个 doGet 方法就是要根据请求，生成响应
        resp.setStatus(200);//设置响应中的状态码
        //这个代码就是把 hello 这个字符串放到 http 响应的 body 中了
        resp.getWriter().write("hello");//响应中的body*/
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       ...
       //1.获取图片的属性信息，并且存入数据库
        ...
        //手动获取一下当前日期，并转化为格式日期  yyyyMMdd => 20200615
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyyMMdd");
        image.setUploadTime(simpleDateFormat.format(new Date()));
        image.setContenType(fileItem.getContentType());
         // MD5  暂时先不去计算
